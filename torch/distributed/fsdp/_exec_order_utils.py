...
class _ExecOrderData:
    ...
    def get_handle_to_forward_prefetch(
        ...
        keys to prefetch, then this returns an empty :class:`list`.
        """
        current_index = current_handles_key.pre_forward_order_index
        if current_index is None:
            return None
        target_index = current_index + 1
    ...
    def record_pre_forward(self, handle: FlatParamHandle, is_training: bool) -> None:
        ...
        if not self.is_first_iter or handle.pre_forward_order_index:
            return
        index = len(self.handles_pre_forward_order)
        handle.pre_forward_order_index = index
